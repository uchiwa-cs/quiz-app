...
const QuizResult: React.FC<QuizResultProps> = ({ quiz }) => {
  //正解数を取得
  const correctNum = quiz.questions.reduce((correctNum, question) => {
  ...
  //スコア計算
  let score = (correctNum / quiz.questions.length) * 100;
  score = Math.round(score);

  //不正回数を取得
  const unCorrectNum = quiz.questions.length - correctNum;

  //経過時間を取得
  const elapsedTime = formatTimeDelta(
    differenceInSeconds(
